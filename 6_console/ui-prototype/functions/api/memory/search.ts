...
export const onRequest: PagesFunction<Env> = async ({ request, env }) => {
  ...
  // ---- Parse request body ----
  let payload: MemorySearchRequest;
  try {
    payload = (await request.json()) as MemorySearchRequest;
  } catch {
  ...
  if (!query) {
    return json({ ok: false, error: "query field is required." }, { status: 400 });
  }
  ...
  try {
    const adapterResponse = await fetch(`${adapterUrl}/memory/search`, {
      method: "POST",
      headers: {
