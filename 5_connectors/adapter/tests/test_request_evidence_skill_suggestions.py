...
class _DummyMeter:
    def __init__(self, request_id: str, task_type: str):
        ...
        self.query_shape = "mixed"
        self.query_chars = 24
        self.query = "need decision with checks"
        self.baseline_chars = 100
        self.actual_chars = 80
        self.saved_chars = 20
        self.baseline_tokens_estimate = 100
        self.actual_tokens_estimate = 80
        self.saved_tokens_estimate = 20
        self.savings_ratio = 0.2
        self.packed_memory_count = 1
        self.local_cards_used = 1
        self.remote_candidates_considered = 0
        self.remote_candidates_skipped = 0
        self.remote_used_count = 0
        self.skipped_remote_reason = None
        self.coverage_satisfied = True
        self.packing_enabled = True
        self.abstract_preferred = False
        self.dedup_applied = True
        self.task_type = task_type
        self.context_bypass = False
        self.bypassed_context_tokens = 0
        self.matched_keywords = []
        self.candidate_memories = [{"content": "a", "score": 0.9}]
...
def test_request_evidence_includes_skill_suggestions_from_compile_meta():
    request_id = "req-skill-hit"
    ...
    assert payload["skill_suggestions"][0]["skill_id"] == "checks"


def test_request_evidence_uses_empty_list_when_compile_has_no_suggestions():
...
def test_request_evidence_implementation_task_keeps_skill_suggestions_empty():
...
def test_request_evidence_compile_skipped_route_disabled_does_not_fabricate_suggestions():
    request_id = "req-route-disabled"
    meter = _DummyMeter(request_id=request_id, task_type="decision")
    payload = _build_payload(
        request_id,
        meter,
