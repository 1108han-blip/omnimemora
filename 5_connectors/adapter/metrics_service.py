...
def _default_overview_meters(meters: List[Any], include_internal: bool = False, include_task_non_value: bool = False) -> List[Any]:
    """
    ...
    4. By default, only include value_qualified requests in KPI metrics.
       Use include_task_non_value=True to also include task_non_value (for diagnostics).
    """
    ...
    if not include_task_non_value:
        # Default: only value_qualified requests count toward KPI
...
def get_recent_requests(tenant: str, limit: int = 20, include_internal: bool = False, value_qualified_only: bool = True) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        value_qualified_only: If True (default), only include value_qualified requests.
                              If False, include task_non_value requests as well (for diagnostics).
...
def compute_core_capabilities(tenant: str) -> Dict[str, Any]:
    ...
    Default KPI filtering uses value_qualified (not is_real_request).
    Internal requests are excluded; task_non_value requests are shown in
    non_value_count but do not contribute to card metrics.
    """
    meters = _collect_meters_24h(tenant)
