...
This repository presents the public GitHub edition under the `OmniMemora` product name. Upstream technical names such as `OpenViking` and `memory-openviking` may still appear where they are required for attribution, component identity, or integration context.
...
New to OmniMemora? These documents will guide you:
...
Ready to move to production? OmniMemora Pro and Enterprise editions provide:
...
OmniMemora doesn't solve "can we store a memory" -- it solves "how should an agent manage memory." In real-world usage, the hardest problems usually aren't the write interface itself, but questions like:
...
OmniMemora uses a three-tier distribution strategy to match different stages of real-world needs:
