...
func TestAttachClaudeRoutesAnthropicBaseURLThroughProduct(t *testing.T) {
	tmpHome := t.TempDir()
  ...
  "theme": "light"
	...
	if envCfg["ANTHROPIC_BASE_URL"] != ProductAdapterAnthropicEndpoint() {
		t.Fatalf("expected ANTHROPIC_BASE_URL=%q, got %#v", ProductAdapterAnthropicEndpoint(), envCfg["ANTHROPIC_BASE_URL"])
	}
