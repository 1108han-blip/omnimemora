...
三实例恢复验证（2026-04-12）— `THREE_INSTANCE_RECOVERY_TEST_PLAN.md` 执行过程中发现

### 额外修复

#### X-1: `meter_store.py` — `store_meter()` 类型混乱

**问题**: `store_meter()` 接受 dict 或 `TokenSavingsMeter`，但将 dict 直接存入 `_meter_store[request_id]`。后续 `get_meter()` 返回 dict，调用 `.to_dict()` 时报错：`AttributeError: 'dict' object has no attribute 'to_dict'`。

**影响**: `/requests/{request_id}/meter` endpoint 返回 500 错误

**修复** (`meter_store.py:152-178`):
```python
# Before — dict 直接存入 in-memory store
if isinstance(meter, dict):
    _meter_store[request_id] = meter
    _usage_aggregates[tenant].append(meter)

# After — dict 转换为 TokenSavingsMeter 后再存储
if isinstance(meter, dict):
    try:
...
**问题**: `result.meter_artifact["tenant"]` 未设置。`optimize_context()` 返回的 meter 中 `tenant="engine"`（engine.py 硬编码），导致 `usage` 按真实租户（`access.tenant_id`）聚合时 `request_count` 始终为 0。

**影响**: Usage 统计不可用，无法按 agent/tenant 分组
...
# After
result.meter_artifact["request_id"] = request_id
result.meter_artifact["tenant"] = access.tenant_id  # 使用真实 tenant
result.meter_artifact["matched_keywords"] = matched_keywords
store_meter(result.meter_artifact)
```
...
```json
// 修复前
{ "request_count": 0, "saved_tokens_total": 0 }

// 修复后
{ "request_count": 1, "saved_tokens_total": 44,
  "by_agent": [{"agent": "claude_code", "saved_tokens": 44, "savings_ratio": 0.407}] }
```

### 恢复测试结果（2026-04-12 22:47 UTC）

| 测试 | 结果 |
|------|------|
| T1: Go Runtime 8765 | ✅ UP (v1.0.0, sqlite, 19 memories) |
| T2: Adapter 18011 | ✅ UP (healthy) |
| T3: Query Path | ✅ 返回 packed_context，saved_tokens=46 |
| T8: Implementation Bypass | ✅ bypass=True, packed_context="" |
| T9: Decision Non-Bypass | ✅ bypass=False, packed_context 有内容 |
| 三实例闭环 (openclaw/codex/claude_code) | ✅ 全部通过 |
| 写入 + 查询 闭环 | ✅ mem_19b96094 写入成功并可查询 |
...
## Post-Audit Recovery Fixes (Instance Validation Phase)
