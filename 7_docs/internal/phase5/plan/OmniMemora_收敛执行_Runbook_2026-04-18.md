...
- `M3` 候选实例：`18012/18765` 启动已验证；`openclaw` 的 `enable/disable` 已完成候选实例落盘闭环；`route=on` 已在候选实例下进入 `runtime_compile`
  ...
  - `M4` 最新证据：`RECORD-B-010`、`RECORD-B-011`、`RECORD-B-012`、`RECORD-B-013`
...
- [x] `uninstall/detach -> restore original config` 已有候选实现测试证据
...
- [x] `claude_code` 在候选实例隔离 `HOME` 下已在线验证 `install -> uninstall -> restore original config`
- [x] `cursor` 在候选实例隔离 `HOME` 下已在线验证 `install -> uninstall -> restore original config`
...
- [x] 已追加新的实例声明记录：外部运行实例不适合作为 M3 控制面验收对象
- [x] 已确认：M3 若继续推进，只能绑定仓库候选实例
- [x] 已解除候选实例启动阻塞
- [x] 已追加“仓库候选实例已显式启动”的记录
