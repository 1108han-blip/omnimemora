...
- 2026-05-13: TI-033 running fix added metadata-only sampling for Anthropic streaming responses through `18011`. Streaming responses are not buffered or stored; the sample records a compact placeholder response so DoloToken can show Claude Code and other streaming agents in the `18081` report with `local_estimated` / `compatible_estimate` labels. Running validation: a real streamed Claude Code-shaped request returned HTTP `200`, `18011 /compile/events` showed `agent_id=claude_code` for request `bdb7bbcf231f`, and `18081 /audit/summary` showed `claude_code` in `top_agents`.
