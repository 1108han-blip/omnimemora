"""Guarded rollback tool for the OpenViking commercialization baseline."""
...
from ov_enterprise_common import ResultRecord


def main() -> int:
    parser = argparse.ArgumentParser(description="OpenViking commercialization rollback")
    ...
    parser.add_argument("--report-path", type=Path, default=DEFAULT_ROLLBACK_REPORT)
    ...
    parser.add_argument("--execute", action="store_true", help="Apply the rollback plan")
    args = parser.parse_args()

    started_ms = monotonic_ms()
    run_id = make_run_id("rollback")
    ...
    runtime_window_execute: dict[str, object] | None = None
    runtime_window_post: dict[str, object] | None = None
    ...
    checks.append(
        ResultRecord(
            ...
            "Rollback sources exist in the selected backup snapshot"
            if restore_sources_exist
            else "One or more rollback sources are missing from the selected backup snapshot",
    ...
    target_writable_details: list[dict[str, object]] = []
    ...
    target_record = ResultRecord(
        ...
        "Rollback targets are writable" if target_writable else "One or more rollback targets are not writable",
    ...
    execute_items.append(
        ResultRecord(
            ...
            "pass",
            "Directory-level rollback rules are active for execute mode",
    ...
    execute_items.append(
        ResultRecord(
            ...
            "pass",
            "Runtime-sensitive directories are excluded from directory-level rollback",
            {
    ...
    if args.execute and status != "fail":
        ...
        try:
            ...
            safety_snapshot_dir = args.safety_backup_root / f"pre-rollback-{stamp}"
            ...
            for op in operations:
                ...
                if target.exists():
                    ...
                    append_execution_event(
                        execution_trace,
                        "snapshot_target_before_rollback",
                        "pass",
            ...
            write_json_report(
                ...
                {
                    **report_metadata("ov-enterprise-pre-rollback-backup", run_id, started_ms),
                    "status": "pass",
                    "mode": "execute",
            ...
            checks.append(ResultRecord("execute_rollback", "fail", f"Rollback execution failed: {exc}"))
            append_execution_event(
                execution_trace,
                "execute_rollback",
                "fail",
            ...
            post_phase = "post_execute" if not any(record.id == "execute_rollback" and record.status == "fail" for record in checks) else "post_execute_failure"
            support_trace.append(
                support_trace_checkpoint(
                    post_phase,
    ...
    if safety_snapshot_dir:
        postcheck_items.append(
            ResultRecord(
                "safety_snapshot",
                "pass",
                "Pre-rollback safety snapshot was created",
                {"path": str(safety_snapshot_dir)},
            )
        )
    if args.execute:
    ...
    report = {
        **report_metadata("ov-enterprise-rollback", run_id, started_ms),
        "status": status,
        "mode": "execute" if args.execute else "dry-run",
        **execute_reason,
        "summary": {
